...
def FREM(K, X, max_iter = 200, rand_initialization = False, version = 1, max_iter_fp = 20):
    ...
    while not(convergence) and  ite<max_iter:
        ...
        while convergence and k<K:
            convergence = convergence and math.sqrt(np.inner(theta_new['mu'][k]-theta_old['mu'][k], theta_new['mu'][k]-theta_old['mu'][k]))/p < 10**(-6)
            convergence = convergence and ((np.linalg.norm(theta_new['Sigma'][k]-theta_old['Sigma'][k], ord='fro')/(p**2)) < 10**(-6))
